...
class ModelTrainer:
    ...
    def get_model_object_and_report(self, train: np.array, test: np.array) -> Tuple[object, object]:
        ...
        Description :   This function trains a XGBRegressor with specified parameters
        ...
        try:
            ...
            metric_artifact = RegressionMetricArtifact(r2_score=r2)
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        ...
        try:
            ...
            # Check if the model's accuracy (r2) meets the expected threshold
            if metric_artifact.r2_score < self.model_trainer_config.expected_accuracy:
                logging.info(f"Model score {metric_artifact.r2_score} is below threshold {self.model_trainer_config.expected_accuracy}")
                # We can choose to raise exception or just warn. Raising exception stops pipeline.
                # raise Exception("No model found with score above the base score")
                logging.warning("Model score is low but saving anyway for inspection (or raise exception here)")
